...
locals {
  ...
  cluster_name = "eks-deployment-sandbox"
  ...
  current_stage = "stage_05_monitoring"  # Options: stage_01_vpc, stage_02_cluster, stage_03_nodes, stage_04_addons, stage_05_monitoring
  ...
  environment_config = {
    ...
    enable_monitoring   = false  # Disabled for sandbox to save costs
  }
  ...
  # DRY: Use centralized stage configuration from root
  # Stage configuration matrix is now defined in root terragrunt.hcl
  # This eliminates duplication across environments
}
...
inputs = {
  ...
  # DRY: Stage Control - Use centralized stage configuration from root
  # Note: stage_config_matrix is defined in parent terragrunt.hcl and inherited
  ...
  # Bastion Configuration (disabled for sandbox)
  ...
  # endpoint_public_access           = false  # More secure for sandbox
  ...
  eks_create_private_access_sg     = false  # No bastion in sandbox
  eks_private_access_cidrs         = []
  eks_additional_security_group_ids = []
  
  # Encryption (disabled for sandbox to reduce complexity)
  ...
  capacity_type              = "SPOT"  # Cost optimization for sandbox
  disk_size                  = local.environment_config.disk_size
  
  # Addons Configuration (stage-aware) - Using DRY stage configuration
  enable_vpc_cni        = local.current_stage == "stage_04_addons" || local.current_stage == "stage_05_monitoring"
  enable_kube_proxy     = local.current_stage == "stage_04_addons" || local.current_stage == "stage_05_monitoring"
  enable_coredns        = local.current_stage == "stage_04_addons" || local.current_stage == "stage_05_monitoring"
  enable_ebs_csi_driver = local.current_stage == "stage_04_addons" || local.current_stage == "stage_05_monitoring"
  create_irsa_roles     = local.current_stage == "stage_04_addons" || local.current_stage == "stage_05_monitoring"
  
  # Monitoring Configuration (stage-aware with cost optimization)
  # FIXED: Enable monitoring for testing (ignoring cost optimization)
  enable_prometheus         = false  # Temporarily disable Prometheus due to Helm requirement
  enable_kong              = false  # Disabled for sandbox
  monitoring_namespace     = "monitoring"
  grafana_admin_password   = "sandbox-admin-123"  # Use stronger password in prod
  prometheus_storage_size  = "20Gi"   # Reduced for sandbox
  grafana_storage_size     = "5Gi"    # Reduced for sandbox
  grafana_ingress_enabled  = false   # Disabled for sandbox
  grafana_ingress_host     = "grafana-sandbox.local"
